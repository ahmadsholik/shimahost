...
switch($_GET[act]){
  ...
  default:
	...
	<div class='container'>
	<ul class='nav'>
	<li><div style='color:white;  padding-top:9px; text-align:left;'><i class='icon-pencil icon-white'></i>Artikel</div></li>
	</ul>
	</div>
	</div>
	</div>
	<h6><input type=button value='Tambah Artikel' onclick=\"window.location.href='?module=berita&act=tambahberita';\" class='btn btn-success'>
	<div class='pull-right'>
	<form method=get action='$_SERVER[PHP_SELF]' id=paging>
	<input type=hidden name=module value=berita>
	<div class='input-append'><input type=text name='kata' id='appendedInputButton' placeholder='Masukan Judul Berita...'>
	<button type=submit class='btn btn-primary'><i class='icon-search icon-white'></i> Cari</button></div>
	</form> </div>";
    if (empty($_GET['kata'])){
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tbody><tr><td>$no</td>
			...
			<li><a href=\"$aksi?module=berita&act=hapus&id=$r[id_berita]&namafile=$r[gambar]\"
							onClick=\"return confirm('Apakah Anda benar-benar mau menghapusnya?')\">
			<i class='icon-trash icon'></i> Hapus Data</a></li>
			</ul>
			</div></td>
			</tr></tbody>";
      $no++;
    ...
    echo "<div class='pager'>$linkHalaman</div><br></h6></div>";
    break;    
    }
    else{
    echo "<table class='table table-condensed' width=100%>  
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
			...
			<li><a href=?module=berita&act=editberita&id=$r[id_berita]><i class='icon-pencil icon'></i> Edit Data</a></li>
			<li><a href=\"$aksi?module=berita&act=hapus&id=$r[id_berita]&namafile=$r[gambar]\"
							onClick=\"return confirm('Apakah Anda benar-benar mau menghapusnya?')\"><i class='icon-trash icon'></i> Hapus Data</a></li>
			</ul>
			</div></td>
			</tr>";
      $no++;
    ...
    $jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
    $linkHalaman = $p->navHalaman($_GET[halaman], $jmlhalaman);
      echo "<div id='pagination'>$linkHalaman</div><br>";
    break;    
    }
  case "tambahberita":
    echo "<div class='well'>
   ...
   while ($t=mysql_fetch_array($tag)){
	  ...
	  echo"</div>
		   <div class='pull-right'><input type=submit value=Simpan class='btn btn-primary'>
		   <input type=button value=Batal onclick=self.history.back() class='btn btn-danger'></div>
		   </div>
		   </form>
		   </div>";
     break;
...
<li><div style='color:white;  padding-top:9px; text-align:left;'><i class='icon-pencil icon-white'></i> Edit Berita</div></li>
      ...
      echo "<label>Headline</label><input type=radio name='headline' value='Y'>Y  
			...
			echo"$d </div><div class='pull-right'><input type=submit value=Update class='btn btn-primary'> <input type=button value=Batal onclick=self.history.back() class='btn btn-danger'></div>
        </div>";          
    echo  "</form></div>";
    break;  
}
